...
void BpeMvnCorScr_updateRP1(gsl_vector *beta1,
                            gsl_vector *xbeta1,
                            gsl_vector *gamma,
                            gsl_vector *V1,
                            gsl_vector *lambda1,
...
void BpeMvnCorScr_updateRP2(gsl_vector *beta2,
                            gsl_vector *xbeta2,
                            double nu2,                            
                            gsl_vector *gamma,
                            gsl_vector *V2,
...
void BpeMvnCorScr_updateRP3(gsl_vector *beta3,
                            gsl_vector *xbeta3,
                            double nu3,
                            gsl_vector *gamma,
                            gsl_vector *V3,
...
void BpeMvnCorScr_updateBH1(gsl_vector *lambda1,
                            gsl_vector *s1,
                            gsl_vector *xbeta1,
                            gsl_vector *gamma,
                            gsl_vector *V1,
...
void BpeMvnCorScr_updateBH2(gsl_vector *lambda2,
                            gsl_vector *s2,
                            gsl_vector *xbeta2,
                            gsl_vector *gamma,
                            gsl_vector *V2,
...
void BpeMvnCorScr_updateBH3(gsl_vector *lambda3,
                            gsl_vector *s3,
                            gsl_vector *xbeta3,
                            gsl_vector *gamma,
                            gsl_vector *V3,
...
void BpeMvnCorScr_updateSP1(double *mu_lam1,
                    double *sigSq_lam1,
                    gsl_vector *lambda1,
                    gsl_matrix *Sigma_lam1,
                    gsl_matrix *invSigma_lam1,
...
void BpeMvnCorScr_updateSP2(double *mu_lam2,
                    double *sigSq_lam2,
                    gsl_vector *lambda2,
                    gsl_matrix *Sigma_lam2,
                    gsl_matrix *invSigma_lam2,
...
void BpeMvnCorScr_updateSP3(double *mu_lam3,
                    double *sigSq_lam3,
                    gsl_vector *lambda3,
                    gsl_matrix *Sigma_lam3,
                    gsl_matrix *invSigma_lam3,
...
void BpeMvnCorScr_updateFP_Gibbs(gsl_vector *gamma,
                                 double theta,
                                 gsl_vector *xbeta1,
                                 gsl_vector *xbeta2,
                                 gsl_vector *xbeta3,
                                 ...
                                 gsl_vector *cluster)
{
    ...
    for(i = 0; i< n; i++)
    {
        ...
        gamma_rate  = BpeMvnCorScr_wFunc(i, xbeta1, xbeta2, xbeta3, lambda1, lambda2, lambda3, jj, V1, V2, V3, s1, s2, s3, J1, J2, J3, survTime1, survTime2) + 1/theta;
...
void BpeMvnCorScr_updateDP(gsl_vector *gamma,
              double *theta,
...
void BpeMvnCorScr_updateBI1(gsl_vector *s1,
                            int *J1,
                            int *accept_BI1,
                            gsl_vector *survTime1,
                            gsl_vector *survEvent1,
...
void BpeMvnCorScr_updateDI1(gsl_vector *s1,
                    int *J1,
                    int *accept_DI1,
                    gsl_vector *survTime1,
                    gsl_vector *survEvent1,
...
void BpeMvnCorScr_updateBI2(gsl_vector *s2,
                    int *J2,
                    int *accept_BI2,
                    gsl_vector *survTime1,
                    gsl_vector *survTime2,
...
void BpeMvnCorScr_updateDI2(gsl_vector *s2,
                    int *J2,
                    int *accept_DI2,
                    gsl_vector *survTime1,
                    gsl_vector *survTime2,
...
void BpeMvnCorScr_updateBI3(gsl_vector *s3,
                    int *J3,
                    int *accept_BI3,
                    gsl_vector *survTime1,
                    gsl_vector *survTime2,
                    ...
                    double s3_max)
{
    ...
    for(i = 0; i < n; i++)
    {
        ...
        if(gsl_vector_get(case11, i) == 1)
        {
            if(j_old == 0 && gsl_vector_get(survTime2, i) <= gsl_vector_get(s3, 0))
            {
                logLH += gsl_vector_get(lambda3, j_old);
            }
            if(j_old != 0 && gsl_vector_get(survTime2, i) > gsl_vector_get(s3, j_old-1) && gsl_vector_get(survTime2, i) <= gsl_vector_get(s3, j_old))
            {
                logLH += gsl_vector_get(lambda3, j_old);
            }
        }
        ...
        if(j_old > 0)
        {
            Del = c_max(0, (c_min(gsl_vector_get(s3, j_old), gsl_vector_get(survTime2, i)) - c_max(gsl_vector_get(s3, j_old-1), gsl_vector_get(survTime1, i))));
        }
        if(j_old == 0)
        {
            Del = c_max(0, (c_min(gsl_vector_get(s3, j_old), gsl_vector_get(survTime2, i)) - c_max(0, gsl_vector_get(survTime1, i))));
        }
        if(Del > 0)
        {
    ...
    for(j = j_old; j < j_old + 2; j++)
    {
        for(i = 0; i < n; i++)
        {
            ...
            if(j > 0)
            {
                Del = c_max(0, (c_min(gsl_vector_get(s_new, j), gsl_vector_get(survTime2, i)) - c_max(gsl_vector_get(s_new, j-1), gsl_vector_get(survTime1, i))));
            }
            if(j == 0)
            {
                Del = c_max(0, (c_min(gsl_vector_get(s_new, j), gsl_vector_get(survTime2, i)) - c_max(0, gsl_vector_get(survTime1, i))));
            }
            if(Del > 0)
            {
                    ...
                    int J3_max)
{
    ...
    for(i = 0; i < n; i++)
    {
        ...
        if(j_old > 0)
        {
            Del = c_max(0, (c_min(gsl_vector_get(s_new, j_old), gsl_vector_get(survTime2, i)) - c_max(gsl_vector_get(s_new, j_old-1), gsl_vector_get(survTime1, i))));
        }
        if(j_old == 0)
        {
            Del = c_max(0, (c_min(gsl_vector_get(s_new, j_old), gsl_vector_get(survTime2, i)) - c_max(0, gsl_vector_get(survTime1, i))));
        }
        if(Del > 0)
        {
...
void BpeMvnCorScr_updateMP(gsl_vector *beta2,
                       gsl_vector *beta3,
                       double *alpha2,
                       double *alpha3,
                       double *kappa2,
...
void BpeMvnCorScr_updateCP1_amcmc(gsl_vector *beta1,
                                    gsl_vector *gamma,
                                    gsl_vector *V1,
                                    gsl_vector *V2,
                                    gsl_vector *V3,
...
void BpeMvnCorScr_updateCP2_amcmc(gsl_vector *beta2,
                        double nu2,
                        gsl_vector *gamma,
                        gsl_vector *V1,
                        gsl_vector *V2,
...
void BpeMvnCorScr_updateCP3_amcmc(gsl_vector *beta3,
                                  double nu3,
                        gsl_vector *gamma,
                        gsl_vector *V1,
                        gsl_vector *V2,
    ...
    for(j = 0; j < J; j++)
    {
        ...
        for(i = startInx; i < endInx; i++)
        {
            ...
            for(k = 0; k < K3+1; k++)
            {
                if(k > 0)
                {
                    Del = c_max(0, (c_min(gsl_vector_get(s3, k), gsl_vector_get(survTime2, i)) - c_max(gsl_vector_get(s3, k-1), gsl_vector_get(survTime1, i))));
                }
                if(k == 0)
                {
                    Del = c_max(0, (c_min(gsl_vector_get(s3, k), gsl_vector_get(survTime2, i)) - c_max(0, gsl_vector_get(survTime1, i))));
                }
...
void BpeMvnCorScr_updateVP(gsl_vector *V1,
                             gsl_vector *V2,
                             gsl_vector *V3,
                             gsl_matrix *Sigma_V,
                             double rho_v,
