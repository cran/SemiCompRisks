/*
 ...
 R CMD SHLIB BweibMvnCorScr.c BweibMvnCorScr_Updates.c BweibMvnCorScr_Utilities.c -lgsl -lgslcblas
                        ...
                        double moveVec[])
{
    ...
    for(MM = 0; MM < *numReps; MM++)
    {
        ...
        if(move == 1)
        {
            BweibMvnCorScr_updateRP1(beta1, &alpha1, &kappa1, gamma, V1, survTime1, survEvent1, cluster, survCov1, accept_beta1);
        }
        ...
        if( ( (MM+1) % *thin ) == 0 && (MM+1) > (*numReps * *burninPerc))
        {
            ...
            for(i = 0; i < *n; i++)
            {
                BweibMvnCorScr_logMLH_i(i, beta1, beta2, beta3, alpha1, alpha2, alpha3, kappa1, kappa2, kappa3, theta, V1, V2, V3, survTime1, survTime2, survEvent1, survEvent2, case01, case11, survCov1, survCov2, survCov3, cluster, &invLHval);
