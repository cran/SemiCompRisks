/*
 ...
 R CMD SHLIB BpeDpCorScrSM.c BpeDpCorScrSM_Updates.c BpeDpCorScrSM_Utilities.c -lgsl -lgslcblas
...
/* */
void BpeDpCorScrSMmcmc(double survData[],
                      int *n,
                      int *p1,
                      int *p2,
                      int *p3,
                       ...
                       double moveVec[])
{
    ...
    gsl_vector * mu0 = gsl_vector_calloc(3);
    gsl_vector_set(mu0, 0, hyperParams[14]);
    gsl_vector_set(mu0, 1, hyperParams[15]);
    gsl_vector_set(mu0, 2, hyperParams[16]);
    
    double zeta0    = hyperParams[17];
    
    gsl_matrix *Psi0 = gsl_matrix_calloc(3,3);
    for(i = 0; i < 3; i++)
    {
        for(j = 0; j < 3; j++)
        {
            gsl_matrix_set(Psi0, j, i, hyperParams[18 + i*3 + j]);
        }
    }
    
    double rho0    = hyperParams[27];
    double aTau    = hyperParams[28];
    double bTau    = hyperParams[29];
    ...
    for(i = 0; i < *J; i++)
    {
        gsl_vector_set(c, i, startValues[*p1 + *p2 + *p3 + 10 + *n + (K1+1)*2 + (K2+1)*2 + (K3+1)*2 + 2 + *J*2 + *J + i]);
    }
    
    double tau = startValues[*p1 + *p2 + *p3 + 10 + *n + (K1+1)*2 + (K2+1)*2 + (K3+1)*2 + 2 + *J*2 + *J + *J];
    ...
    double pRP1, pRP2, pRP3, pBH1, pBH2, pBH3, pSP1, pSP2, pSP3, pBI1, pBI2, pBI3, pDI1, pDI2, pDI3, pFP, pDP, pMP, pCP, choice;
    int move, numUpdate;
    ...
    for(MM = 0; MM < *numReps; MM++)
    {
        ...
        double probSub = (1 - pMP - pDP - pCP - pBI1 - pBI2 - pBI3 - pDI1 - pDI2 - pDI3)/(numUpdate-9);
        ...
        /* move: 1=RP1, 2=RP2, 3=RP3, 4=BH1, 5=BH2, 6=BH3, 7=SP1, 8=SP2, 9=SP3, 10=FP,
         11=DP, 12=BI1, 13=BI2, 14=BI3, 15=DI1, 16=DI2, 17=DI3, 18=CP, 19=MP */
        ...
        if(choice > pRP1+pRP2+pRP3+pBH1+pBH2+pBH3+pSP1+pSP2+pSP3+pFP+pDP+pBI1+pBI2+pBI3+pDI1+pDI2+pDI3+pCP) move = 19;
;
        ...
        if(move == 1)
        {
            BpeDpCorScrSM_updateRP1(beta1, xbeta1, gamma, V1, lambda1, s1, survTime1, survEvent1, cluster, survCov1, K1, accept_beta1);
        }
        ...
        if(move == 2)
        {
            BpeDpCorScrSM_updateRP2(beta2, xbeta2, nu2, gamma, V2, lambda2, s2, survTime1, case01, cluster, survCov2, K2, accept_beta2);
        }
        ...
        */
        ...
        if(move == 3)
        {
            BpeDpCorScrSM_updateRP3(beta3, xbeta3, nu3, gamma, V3, lambda3, s3, yStar, case11, cluster, survCov3, K3, accept_beta3);
        }
        ...
        if(move == 4)
        {
            BpeDpCorScrSM_updateBH1(lambda1, s1, xbeta1, gamma, V1, survTime1, survEvent1, cluster, Sigma_lam1, invSigma_lam1, W1, Q1, mu_lam1, sigSq_lam1, K1);
        }
        ...
        if(move == 5)
        {
            BpeDpCorScrSM_updateBH2(lambda2, s2, xbeta2, gamma, V2, nu2, survTime1, survTime2, case01, cluster, Sigma_lam2, invSigma_lam2, W2, Q2, mu_lam2, sigSq_lam2, K2);
        }
        ...
        if(move == 6)
        {
            BpeDpCorScrSM_updateBH3(lambda3, s3, xbeta3, gamma, V3, nu3, yStar, case11, cluster, Sigma_lam3, invSigma_lam3, W3, Q3, mu_lam3, sigSq_lam3, K3);
        }
        ...
        if(move == 7)
        {
            BpeDpCorScrSM_updateSP1(&mu_lam1, &sigSq_lam1, lambda1, Sigma_lam1, invSigma_lam1, a1, b1, K1);
        }
        ...
        if(move == 8)
        {
            BpeDpCorScrSM_updateSP2(&mu_lam2, &sigSq_lam2, lambda2, Sigma_lam2, invSigma_lam2, a2, b2, K2);
        }
        ...
        if(move == 9)
        {
            BpeDpCorScrSM_updateSP3(&mu_lam3, &sigSq_lam3, lambda3, Sigma_lam3, invSigma_lam3, a3, b3, K3);
        }
        ...
        if(move == 10)
        {
            ...
            BpeDpCorScrSM_updateFP_Gibbs(gamma, theta, xbeta1, xbeta2, xbeta3, lambda1, lambda2, lambda3, s1, s2, s3, K1, K2, K3, V1, V2, V3, survTime1, yStar, survEvent1, survEvent2, cluster);
        }
        ...
        if(move == 11)
        {
            BpeDpCorScrSM_updateDP(gamma, &theta, mhProp_theta_var, psi, omega, &accept_theta);
        }
        ...
        if(move == 12)
        {
            BpeDpCorScrSM_updateBI1(s1, &K1, &accept_BI1, survTime1, survEvent1, gamma, xbeta1, V1, cluster, Sigma_lam1, invSigma_lam1, W1, Q1, lambda1, s_propBI1, num_s_propBI1, delPert1, alpha1, c_lam1, mu_lam1, sigSq_lam1, s1_max);
        }
        ...
        if(move == 13)
        {
            BpeDpCorScrSM_updateBI2(s2, &K2, &accept_BI2, survTime1, survTime2, case01, gamma, xbeta2, V2, cluster, Sigma_lam2, invSigma_lam2, W2, Q2, lambda2, s_propBI2, num_s_propBI2, delPert2, alpha2, c_lam2, mu_lam2, sigSq_lam2, s2_max);
        }
        ...
        if(move == 14)
        {
            BpeDpCorScrSM_updateBI3(s3, &K3, &accept_BI3, survTime1, yStar, case11, gamma, xbeta3, V3, cluster, Sigma_lam3, invSigma_lam3, W3, Q3, lambda3, s_propBI3, num_s_propBI3, delPert3, alpha3, c_lam3, mu_lam3, sigSq_lam3, s3_max);
        }
        ...
        if(move == 15)
        {
            BpeDpCorScrSM_updateDI1(s1, &K1, &accept_DI1, survTime1, survEvent1, gamma, xbeta1, V1, cluster, Sigma_lam1, invSigma_lam1, W1, Q1, lambda1, s_propBI1, num_s_propBI1, delPert1, alpha1, c_lam1, mu_lam1, sigSq_lam1, s1_max, K1_max);
        }
        ...
        if(move == 16)
        {
            BpeDpCorScrSM_updateDI2(s2, &K2, &accept_DI2, survTime1, survTime2, case01, gamma, xbeta2, V2, cluster, Sigma_lam2, invSigma_lam2, W2, Q2, lambda2, s_propBI2, num_s_propBI2, delPert2, alpha2, c_lam2, mu_lam2, sigSq_lam2, s2_max, K2_max);
        }
        ...
        if(move == 17)
        {
            BpeDpCorScrSM_updateDI3(s3, &K3, &accept_DI3, survTime1, yStar, case11, gamma, xbeta3, V3, cluster, Sigma_lam3, invSigma_lam3, W3, Q3, lambda3, s_propBI3, num_s_propBI3, delPert3, alpha3, c_lam3, mu_lam3, sigSq_lam3, s3_max, K3_max);
        }
        
  
        
        /* updating cluster-specific random effect and precision parameter of DP prior: V1, V2, V3 and alpha
        ...
        if( ( (MM+1) % *thin ) == 0 && (MM+1) > (*numReps * *burninPerc))
        {
            ...
            for(j = 0; j < (*J * 3); j++) samples_mu[(StoreInx - 1) * (*J * 3) + j] = gsl_vector_get(mu_all, j);
            ...
            for(j = 0; j < (3* *J); j++)
            {
                for(i = 0; i < 3; i++)
                {
                    samples_Sigma[(StoreInx - 1) * (3 * 3 * *J) + j*3 + i] = gsl_matrix_get(Sigma_all, i, j);
                }
            }
            ...
            BpeDpCorScrSM_logMLH(beta1, beta2, beta3, xbeta1, xbeta2, xbeta3, theta, lambda1, lambda2, lambda3, s1, s2, s3, V1, V2, V3, survTime1, survTime2, yStar, survEvent1, survEvent2, case01, case11, survCov1, survCov2, survCov3, cluster, K1, K2, K3, &logLH);
            ...
            for(i = 0; i < *n; i++)
            {
                BpeDpCorScrSM_logMLH_i(i, beta1, beta2, beta3, xbeta1, xbeta2, xbeta3, theta, lambda1, lambda2, lambda3, s1, s2, s3, V1, V2, V3, survTime1, survTime2, yStar, survEvent1, survEvent2, case01, case11, survCov1, survCov2, survCov3, cluster, K1, K2, K3, &invLHval);
        ...
        if(MM == (*numReps - 1))
        {
            ...
            for(i = 0; i < *J; i++) samples_misc[*p1 + *p2 + *p3 + 10 + *n + *n + i] = (int) gsl_vector_get(accept_V, i);
