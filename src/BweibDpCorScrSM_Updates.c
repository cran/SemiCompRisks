    ...
    *nClass_DP = u;
    ...
    for(j = 0; j < J; j++)
    {
        ...
        for(i = startInx; i < endInx; i++)
        {
            ...
            term = -gsl_vector_get(gamma, i) * (kappa3 * pow(gsl_vector_get(yStar, i), alpha3) * exp(LP3 + gsl_vector_get(V3, j)));
            logLH +=  term;
            D1 += term;
            D2 += term;
        ...
        for(i = startInx; i < endInx; i++)
        {
            ...
            term = -gsl_vector_get(gamma, i) * (kappa3 * pow(gsl_vector_get(yStar, i), alpha3) * exp(LP3 + gsl_vector_get(V_prop, 2)));
            logLH_prop +=  term;
            D1_prop += term;
            D2_prop += term;
        }
